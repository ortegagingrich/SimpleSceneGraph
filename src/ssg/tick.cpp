...
SHARED_EXPORT TickRecord *ssg::create_tick_record(int size){
...
SHARED_EXPORT void ssg::delete_tick_record(TickRecord *record){
	if(record != NULL){
...
SHARED_EXPORT float ssg::compute_tick_record_fps(TickRecord *record){
	if(record == NULL){
...
SHARED_EXPORT float ssg::tick(int target_fps, TickRecord *record){
	if(record == NULL){
		if(MASTER_RECORD == NULL){
